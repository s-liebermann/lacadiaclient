...
class BattleChoiceBuilder {
		...
		if (choice.choiceType === 'move') {
			if (!choice.targetLoc && this.requestLength() > 1) {
				...
				if (choosableTargets.includes(this.getChosenMove(choice, this.index()).target)) {
					...
					this.current.hyper = choice.hyper;
					return null;
				}
			}
			...
			if (choice.hyper) this.alreadyHyper = true;
			this.current.move = 0;
	...
	stringChoice(choice: BattleChoice | null) {
		...
		case 'move':
			...
			const boost = `${choice.max ? ' max' : ''}${choice.mega ? ' mega' : ''}${choice.z ? ' zmove' : ''}${choice.tera ? ' terastallize' : ''}${choice.hyper ? ' hyper' : ''}`;
